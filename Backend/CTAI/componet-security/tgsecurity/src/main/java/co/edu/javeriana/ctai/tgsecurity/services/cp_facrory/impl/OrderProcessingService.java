package co.edu.javeriana.ctai.tgsecurity.services.cp_facrory.impl;
...
import co.edu.javeriana.ctai.tgsecurity.services.cp_facrory.interfaces.IOrderProcessingService;
...
public class OrderProcessingService implements IOrderProcessingService {
    ...
    public void startProcessing() {
        ...
        LOGGER.info("Cola de órdenes inicializada");
    }
    ...
    private boolean sendHttpRequest(Order order) {
        try {
            ...
            if (responseEntity.getStatusCode().is2xxSuccessful()) {
                ...
                LOGGER.info("Orden guardada en BD: id-" + orderNumber);
                LOGGER.info("Solicitud HTTP DE LA ORDEN A CP_Factory es exitosa");

                return true;
            } else {
                // La solicitud no fue exitosa
                LOGGER.info("Solicitud HTTP DE LA ORDEN A CP_Factory es no exitosa");
                return false;
            }
        } catch (Exception e) {
            LOGGER.info("Error al realizar la solicitud HTTP a CP_Factory" + e.getMessage());
            return false;
        }
    }
