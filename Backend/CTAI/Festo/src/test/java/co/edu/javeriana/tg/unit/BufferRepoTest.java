...
public class BufferRepoTest {
    ...
    public void testEmptyCRUD() {
        Buffer emptyOrder = new Buffer();
        List<Buffer> createdList = idsList.stream().map(id -> new Buffer())
                .collect(Collectors.toList());
        // Create
        assertThrows(JpaSystemException.class, () -> bufferRepository.save(emptyOrder));
        createdList.forEach(createdElement -> assertThrows(JpaSystemException.class,
                () -> bufferRepository.save(createdElement)));
        // Read
        assertEquals(bufferRepository.count(), 0);
        assertEquals(Optional.empty(), bufferRepository.findById(testID));
        bufferRepository.findAllById(idsList).forEach(element -> assertEquals(Optional.empty(), element));
        // Update
        emptyOrder.setBeltNumber(testID.getBufferNumber());
        assertThrows(JpaSystemException.class, () -> bufferRepository.save(emptyOrder));
        createdList.forEach(createdElement -> {
            createdElement.setBeltNumber(testID.getBufferNumber());
            assertThrows(JpaSystemException.class, () -> bufferRepository.save(createdElement));
        });
        // Delete
        assertDoesNotThrow(() -> bufferRepository.delete(emptyOrder));
